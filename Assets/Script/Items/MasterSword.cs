...
public class MasterSword : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (enemyHealth != null)
        {
            enemyHealth.TakeDamage(10, currentVelocity * 100);
        }
    }
    ...
    void MeanVelocity()
    {
        ...
        if (meanTimeLeft < 0)
        {
            currentVelocity = (transform.root.InverseTransformPoint(transform.position) - lastPos) / Time.deltaTime;
            Debug.Log("currentVelocity " + currentVelocity);

            currentVelocityMag = currentVelocity.magnitude;
            lastPos = transform.root.InverseTransformPoint(transform.position);
